//! This module implements message expansion according to the
//! hash-to-curve RFC
...
pub trait InitExpandMessage<'x>: ExpandMessage<'x> {
    fn init_expand(message: &[u8], dst: &'x [u8], len_in_bytes: usize) -> Self;
}

/// Trait for types implementing expand_message interface for hash_to_field
pub trait ExpandMessage<'x> {
...
impl<'x, H> ExpandMessage<'x> for ExpandMsgXof<H>
where
...
impl<'x, H> InitExpandMessage<'x> for ExpandMsgXmd<'x, H>
where
    H: Digest + BlockInput,
{
    fn init_expand(message: &[u8], dst: &'x [u8], len_in_bytes: usize) -> Self {
...
impl<'x, H> ExpandMessage<'x> for ExpandMsgXmd<'x, H>
where
    H: Digest + BlockInput,
{
    fn read_into(&mut self, output: &mut [u8]) -> usize {
