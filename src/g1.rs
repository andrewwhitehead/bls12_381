...
impl G1Projective {
    ...
    pub fn clear_cofactor(&self) -> G1Projective {
        self - self.mul_by_x()
        //self * Scalar::from(0xd201000000010001u64)
        // self * (Scalar::from(1) + Scalar::from(crate::BLS_X))
    }
    ...
    pub(crate) fn map_isogeny(&self, coeffs: [&[Fp]; 4]) -> G1Projective {
        // unpack input point
        ...
        // let mut f = [0u8; 48];
        // hex::decode_to_slice("11a05f2b1e833340b809101dd99815856b303e88a2d7005ff2627b56cdb4e2c85610c2d5f2e62d6eaeac1662734649b7", &mut f).unwrap();
        // println!("f {:#x?}", Fp::from_bytes(&f).unwrap());
...
fn test_clear_cofactor_b() {
    ...
    for _ in 0..32 {
        let input = G1Projective::random(&mut rng);
        assert_eq!(input.clear_cofactor(), input * h_eff);
    }
}
